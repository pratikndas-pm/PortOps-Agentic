...
def berth_plan():
    ...
    for _, v in df.iterrows():
        ...
        plan.append({
            ...
            "berth": chosen["berth_id"], "cranes": cranes,
            "moves": int(v["moves"]), "mph": round(v["moves"]/hrs, 1)
        })
...
def yard_alloc(total_moves: int = 2000):
    ...
    for _, y in YARD.iterrows():
        ...
        if take > 0:
            ...
            rem -= take
    if rem > 0:
        out.append({"block": "(overflow)", "allocate_slots": int(rem), "type": "IMP"})
...
def crane_for(vessel_name: str):
    ...
    if row.empty:
        return f"Couldn't find vessel '{vessel_name}'. Run 'plan' first or check name.", None
    r = row.iloc[0]
...
def bar(plan):
    ...
    fig.update_layout(margin=dict(l=10, r=10, t=40, b=10), paper_bgcolor="#0d1a26",
                      plot_bgcolor="#0d1a26", font_color="#eaf1f7")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            chat = gr.ChatInterface(
                fn=router, type='messages', autofocus=True, fill_height=True,
                description="Try: 'berth plan', 'list schedule', 'crane plan MSC AURORA', 'yard plan 3000'"
            )
        with gr.Column(scale=1):
...
# expose `demo` (Spaces discovers and runs it)
